...
* **Euler Angles:** Three angles introduced by Leonhard Euler to describe the orientation of a rigid body.
* **Rotation Matrix:** A matrix that is used to perform a rotation in Euclidean space.
* **Translation:** A function that moves every point a constant distance in a specified direction.
...
### Executable Example: Spatial Transformations and Interactivity
...
print("Rotated 90 deg:", np.round(rotated, 5))

translated = translate_2d(rotated, np.array([2.0, 3.0]))
print("Translated by [2, 3]:", translated)
```

#### Interactive Problem-Based Learning Exercise

Use the following widget to explore how changing the translation vector and rotation angle affects the final position of a sensor.
...
def plot_transformation(tx=0.0, ty=0.0, angle_deg=0.0):
    # Initial point
    point = np.array([1.0, 0.0])
    ...
    rotated = rot_mat @ point
    
    # Translation
    translation = np.array([tx, ty])
    final_point = rotated + translation
    
    # Plotting
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.plot([0, point[0]], [0, point[1]], "bo-", label="Original")
    ax.plot([0, rotated[0]], [0, rotated[1]], "go-", label="Rotated")
    ax.plot([translation[0], final_point[0]], [translation[1], final_point[1]], "ro-", label="Rotated + Translated")
...
interact(plot_transformation, 
         tx=FloatSlider(min=-3.0, max=3.0, step=0.5, value=0.0),
         ty=FloatSlider(min=-3.0, max=3.0, step=0.5, value=0.0),
         angle_deg=FloatSlider(min=-180, max=180, step=15, value=45));
```
